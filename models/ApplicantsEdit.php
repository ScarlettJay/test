 ...
 * Page class
 */
class ApplicantsEdit extends Applicants
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ApplicantsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("applicant_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->applicant_id->setQueryStringValue($keyValue);
                $this->applicant_id->setOldValue($this->applicant_id->QueryStringValue);
            } elseif (Post("applicant_id") !== null) {
                $this->applicant_id->setFormValue(Post("applicant_id"));
                $this->applicant_id->setOldValue($this->applicant_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->applicant_id->IsDetailKey) {
            $this->applicant_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->applicant_id->CurrentValue = $this->applicant_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->applicant_id->Required) {
            if (!$this->applicant_id->IsDetailKey && EmptyValue($this->applicant_id->FormValue)) {
                $this->applicant_id->addErrorMessage(str_replace("%s", $this->applicant_id->caption(), $this->applicant_id->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if ($this->registration_id->CurrentValue != "") { // Check field with unique index
            $filterChk = "(`registration_id` = " . AdjustSql($this->registration_id->CurrentValue, $this->Dbid) . ")";
        ...
        if (!$rsold) {
            ...
            // registration_id
            $this->registration_id->setDbValueDef($rsnew, $this->registration_id->CurrentValue, null, $this->registration_id->ReadOnly);
            ...
            // ic_number
            $this->ic_number->setDbValueDef($rsnew, $this->ic_number->CurrentValue, null, $this->ic_number->ReadOnly);
            ...
            // education_level
            $this->education_level->setDbValueDef($rsnew, $this->education_level->CurrentValue, null, $this->education_level->ReadOnly);

            // job_position
            $this->job_position->setDbValueDef($rsnew, $this->job_position->CurrentValue, null, $this->job_position->ReadOnly);

            // certificate
            if ($this->certificate->Visible && !$this->certificate->ReadOnly && !$this->certificate->Upload->KeepFile) {
                $this->certificate->Upload->DbValue = $rsold['certificate']; // Get original value
            ...
            // application_date
            $this->application_date->setDbValueDef($rsnew, UnFormatDateTime($this->application_date->CurrentValue, $this->application_date->formatPattern()), null, $this->application_date->ReadOnly);
            ...
            if ($this->certificate->Visible && !$this->certificate->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->certificate->Upload->FileName)) {
                    ...
                    $this->certificate->setDbValueDef($rsnew, $this->certificate->Upload->FileName, null, $this->certificate->ReadOnly);
                }
            }

 ...
 * Page class
 */
class RegistrationEdit extends Registration
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RegistrationEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("registration_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->registration_id->setQueryStringValue($keyValue);
                $this->registration_id->setOldValue($this->registration_id->QueryStringValue);
            } elseif (Post("registration_id") !== null) {
                $this->registration_id->setFormValue(Post("registration_id"));
                $this->registration_id->setOldValue($this->registration_id->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("RegistrationList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "RegistrationList") {

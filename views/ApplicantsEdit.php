...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fapplicantsedit = new ew.Form("fapplicantsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fapplicantsedit;

    // Add fields
    var fields = currentTable.fields;
    fapplicantsedit.addFields([
        ["applicant_id", [fields.applicant_id.visible && fields.applicant_id.required ? ew.Validators.required(fields.applicant_id.caption) : null], fields.applicant_id.isInvalid],
    ...
    // Form_CustomValidate
    fapplicantsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fapplicantsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fapplicantsedit.lists.education_level = <?= $Page->education_level->toClientList($Page) ?>;
    fapplicantsedit.lists.job_position = <?= $Page->job_position->toClientList($Page) ?>;
    loadjs.done("fapplicantsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fapplicantsedit" id="fapplicantsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->applicant_id->Visible) { // applicant_id ?>
    <div id="r_applicant_id"<?= $Page->applicant_id->rowAttributes() ?>>
        <label id="elh_applicants_applicant_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->applicant_id->caption() ?><?= $Page->applicant_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->applicant_id->cellAttributes() ?>>
<span id="el_applicants_applicant_id">
<span<?= $Page->applicant_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->applicant_id->getDisplayValue($Page->applicant_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="applicants" data-field="x_applicant_id" data-hidden="1" name="x_applicant_id" id="x_applicant_id" value="<?= HtmlEncode($Page->applicant_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_applicants_education_level">
    <select
        ...
        data-select2-id="fapplicantsedit_x_education_level"
...
<script>
loadjs.ready("fapplicantsedit", function() {
    var options = { name: "x_education_level", selectId: "fapplicantsedit_x_education_level" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fapplicantsedit.lists.education_level.lookupOptions.length) {
        options.data = { id: "x_education_level", form: "fapplicantsedit" };
    } else {
        options.ajax = { id: "x_education_level", form: "fapplicantsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_applicants_job_position">
    <select
        ...
        data-select2-id="fapplicantsedit_x_job_position"
        data-table="applicants"
...
<script>
loadjs.ready("fapplicantsedit", function() {
    var options = { name: "x_job_position", selectId: "fapplicantsedit_x_job_position" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fapplicantsedit.lists.job_position.lookupOptions.length) {
        options.data = { id: "x_job_position", form: "fapplicantsedit" };
    } else {
        options.ajax = { id: "x_job_position", form: "fapplicantsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<input type="hidden" name="fa_x_certificate" id= "fa_x_certificate" value="<?= (Post("fa_x_certificate") == "0") ? "0" : "1" ?>">
...
<script>
loadjs.ready(["fapplicantsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fapplicantsedit", "x_application_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
